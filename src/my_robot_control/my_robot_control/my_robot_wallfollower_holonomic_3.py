...
class WallFollower(Node):
    def __init__(self):
        ...
        self.declare_parameter('wall_threshold', 1.0)    # Latent condition threshold
        ...
        self.get_logger().info(
            "WallFollower Holonomic + LATENT SEEK CONDITION."
        )
    ...
    def laser_callback(self, scan):
        ...
        #----------------------------------------------------------
        # RULE 2: FRONT-RIGHT -> Logic Changed to PASS Obstacle
        #----------------------------------------------------------
        elif min_fr_right < self.base_distance:
            # If CRITICALLY CLOSE (< 75% of limit), slide away as before
            if min_fr_right < self.base_distance * 0.75:
                twist.linear.x = self.v_lin * 0.5
                twist.linear.y = self.v_lin
                action = f"FR-RIGHT CRIT ({min_fr_right:.2f}) -> DIAG FRONT-LEFT"
            else:
                # WARNING: Close, but clear. DRIVE FORWARD to pass it.
                twist.linear.x = self.v_lin
                twist.linear.y = 0.0
                action = f"FR-RIGHT PASS ({min_fr_right:.2f}) -> FORWARD"

        #----------------------------------------------------------
        # RULE 3: RIGHT -> Maintain Parallel
        # Only active if wall is WITHIN the search threshold.
        #----------------------------------------------------------
        elif math.isfinite(min_right) and min_right < self.wall_threshold:
        ...
        #----------------------------------------------------------
        elif math.isfinite(min_back_right) and min_back_right < self.wall_threshold:
            twist.linear.x = self.v_lin
            twist.linear.y = -self.v_lin 
        ...
        #----------------------------------------------------------
        # LATENT CONSTANT CONDITION
        # Wall > Threshold (or lost) -> Always go Right
        #----------------------------------------------------------
        else:
            twist.linear.x = self.v_lin
            twist.linear.y = -self.v_lin * 0.5 # Seek Right
            action = "RIGHT > THRESHOLD -> CONSTANT SEEK RIGHT"
